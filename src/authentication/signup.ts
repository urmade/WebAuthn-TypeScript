...
export function registerKey(keyCredentialObject: { [key: string]: any }): number {
	...
	if (!(clientData.type === "webauthn.create")) {
		console.error("The operation specified in the clientDataJSON is not webauthn.create");
		return 403;
	}
	...
	if (process.env.ALLOWEDALGORITHMS && !process.env.ALLOWEDALGORITHMS.split(",").includes(authenticatorData.attestedCredentialData.credentialPublicKey.kty)) {
		console.error("The request used an encryption method that is not allowed by this server");
		return 403;
	}
	...
	if (!(
		...
		console.error("The request doesn't match any known attestation type and can therefore not be processed");
		return 403;
	}
	...
	if(!validAttestationSignature) {
		console.error("The requests attestation signature could not be verified");
		return 403;
	}
	...
	if(storage.isDuplicate(credentialId)) {
		console.error("The credentialId is already in use. Please re-attempt the registration");
		return 418;
	}
